...
\caption{Phylodynamic estimates for the time-varying effective reproductive number $R_e$ (top) and sampling probability (bottom) in Switzerland. $R_e$ estimates are overlaid on estimates generated from case count data \cite{huisman_re_preprint} in grey. Additionally, $R_e$ estimates from the model with a damping factor (blue) is the ``baseline'' $R_e$ before introduction-specific damping once introductions are older than 2-days post sampling.}  
...
\caption{Phylodynamic estimates for the time-varying effective reproductive number $R_e$ in New Zealand. $R_e$ estimates are overlaid on estimates generated from case count data \cite{huisman_re_preprint} in grey. Additionally, $R_e$ estimates from the model with a damping factor (blue) is the ``baseline'' $R_e$ before introduction-specific damping once introductions are older than 2-days post sampling.}  
\label{fig:NZLResults}
\end{figure}

\begin{figure}[tbhp]
\centering
\includegraphics[width=\linewidth]{figures/contact_tracing_factors_no_sampUB.pdf}
