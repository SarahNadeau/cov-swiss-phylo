...
\title{Can we quantify the effects of public health measures on SARS-CoV-2 transmission in Switzerland?}
...
\begin{figure}[tbhp]
\centering
\includegraphics[width=.4\linewidth]{figures/chain_longevity_matrix.png}
\caption{Number of sampled introductions by the month they were fist sampled, in Switzerland. Estimate ranges are between analyses assuming the largest and few introductions. The red color scales with the log of the midpoint of the range. So, darker red means more sampled introductions and lighter red means fewer.}
...
\begin{figure}[tbhp]
\centering
\includegraphics[width=0.8\linewidth]{figures/CT_combined.pdf}
\caption{Estimated slow-down in transmission once introductions are sampled during different time periods in (a) Switzerland and (b) New Zealand. (c) shows the support for including a slow-down factor in the model during each time period. The two facets are for analyses run assuming few or many introductions.}  
...
Next, we seek to interrogate limitations of the applied phylogenetic and phylodynamic methods. Most obviously, estimates on the number, timing, and duration of introductions are very uncertain due to multifurcations in the phylogeny. Others have struggled with the same challenge \cite{Morel2021} and we support their conclusion: one should consider estimates generated across the range of plausible phylogenies, as estimates based on any one phylogeny may be misleading. In our analysis, we present estimate ranges based on two extreme resolutions of multifurcations. This is less computationally intensive, but perhaps overly conservative, compared to generating a larger set of plausible phylogenies as in Bayesian analyses and as \cite{Morel2021} suggested for maximum-likelihood analyses.
...
Finally, we cannot unequivocally attribute the estimated slow-down in transmission upon sampling to test-trace-isolate success. Returning travellers have been implicated in transmitting more than non-travellers \cite{Hodcroft2021}. A passive slow-down in transmission might happen as the virus moves into the non-traveller population. To this end, we repeated our analysis using data from New Zealand, where all travellers were quarantined since TODO. In the period from 15. May to \maxdate, we estimated a similar or stronger slow-down in transmission compared to Switzerland in Summer 2020. Since New Zealand travellers could not mix normally with non-travellers, at least in New Zealand the slow-down appears to be from quarantine mitigating down-stream transmission from introductions. We also noted that contacts of positive cases and members of large transmission chains are more likely to be sampled. This means we might sample bursts of transmission, possibly due to super-spreading, which are again followed by a passive slow-down in transmission once a super-spreading event is over or a super-spreader is no longer infectious. However, in this case we would expect periodic bursts of super-spreading-related samples to occur regardless of whether an introduction is recently sampled or not. We would also expect super-spreading to continue into Fall 2020, when case counts were high in Switzerland, but the slow-down factor is only significant in Summer 2020, when case counts were low. Therefore, we suspect test-trace-isolate and quarantine measures help mitigate onward transmission from introductions when case counts were low in Switzerland and New Zealand. Phrased more conservatively, we robust genomic signal for bursts of transmission corresponding to newly sampled introductions followed by a slow-down in subsequent transmission.

% TODO: Contextualize: other studies on bursty SARS-CoV-2 phylogenies?
% TODO: Contextualize: do we address any points from TTIQ simulation studies?
