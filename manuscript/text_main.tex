...
\title{Genomic evidence for impact of public health measures and summer travel on SARS-CoV-2 transmission dynamics in Switzerland}
...
% Finally, \cite{Ragonnet-Cronin2021} took a different approach by comparing data across many regions. They showed a correlation between death counts and the delay between (phylogenetically-estimated) epidemic start and implementation of strong measures. 
% In this work, we employ a combination of phylogenetic and phylodynamic methods. with the aim of quantifying transmission dynamics across a longer timescale. 
...
\caption{Illustration of how transmission rate damping is modelled. (A) shows a background time-varying reproductive number that might be proposed at one step of the inference. The boxes in (A) highlight time-periods during which two example introductions (B) and (C) are spreading. The genome data sampled from these introductions are illustrated as red dots. The viral phylogenies connecting these samples are likewise proposed in the inference. Finally, a transmission damping factor of 20\% is proposed. The damping factor is applied for each introduction 2 days after the first genome sample (this is illustrated with a dashed line). In the end, the likelihood of the sampled genome data is calculated given the ``applied'' effective reproductive number specific to each introduction (B & C, bottom).}  
