...
\input{tables/origins_table_posterior_v_prior}
...
\input{tables/origins_table_posterior_v_exposures}
% Caption and label added directly to table .tex file:
...
We can compare the number of reported exposures from countries during periods when these countries were on the quarantine versus off it \ref{fig:chain-origins-by-quarantine-status}, but these data appear include individuals who succesfully quarantined. 
...
\caption{Recorded exposure locations and estimated transmission chain origin locations contrasted with periods when travellers were under quarantine-on-arrival orders.}  
