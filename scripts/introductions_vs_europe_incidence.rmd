...
case_data_base <- cases_raw %>%
        filter(countryterritoryCode != "CHE",
               countryterritoryCode %in% c("ITA", "FRA", "DEU")) %>%
        group_by(date) %>%
        summarize(cases = sum(cases), .groups = "drop") %>%
        arrange(date) %>%
        mutate(weekly_avg_new_cases = zoo::rollmean(cases,7, align = "center", na.pad = T)) %>%
...
p3 <- ggplot(
        ...
        shared_theme +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5), legend.position = c(0.8, 0.8), legend.spacing.y = unit(0, "cm")) +
        labs(x = element_blank(), y = "Number of new introductions")
ggsave(plot = p3, file = "figures/introductions_vs_indicence_model.png",
...
```{r}
prediced_introductions_averted <- data %>% filter(date >= as.Date("2020-03-13")) %>%
...
prediced_introductions_averted_as_per <- data %>% filter(date >= as.Date("2020-03-13")) %>%
...
Estimate by how much lockdown shorted the persistence of introductions
```{r}
...
for (idx in seq_along(dates)) {
  ...
  chains_on_date_data <- chain_summary_stats %>%
          filter(first_sample_date <= start_date, last_sample_date >= start_date) %>%  # chains that start either this day or in the next 7 and end after this day
          mutate(truncated_chain = last_sample_date > as.Date("2020-12-01") - 30) %>%  # persistence possibly truncated by sampling window
          ...
          summarize(n = n(),
                    frac_truncated = sum(truncated_chain) / n(),
...
p <- ggplot(data = persistence_results %>% filter(date <= as.Date("2020-07-01")), aes(x = date)) +
        geom_ribbon(aes(ymin = Q1, ymax = Q3), alpha = 0.3) +
        geom_line(aes(y = median, color = n), size = 2) +
        scale_color_gradient(low = "green", high = "red", name = "Number of onging\nintroductions", trans = "log10") +
        # geom_line(aes(y = frac_truncated * 100)) +
        facet_grid(chains_assumption ~ ., scales = "free_y", labeller = labeller(chains_assumption = chain_assumption_labs)) +
        ...
        labs(x = element_blank(), y = "Persistence from date (days)")
ggsave(filename = "figures/persisence_over_time.png",
       width = single_col_width, height = single_col_width, units = "cm")

p <- ggplot(data = persistence_results, aes(x = date)) +
        geom_ribbon(aes(ymin = Q1, ymax = Q3), alpha = 0.3) +
        geom_line(aes(y = median, color = n), size = 2) +
        scale_color_gradient(low = "green", high = "red", name = "Number of onging\nintroductions", trans = "log10") +
        geom_line(aes(y = as.integer(as.Date("2020-12-01") - date))) +
        facet_grid(chains_assumption ~ ., scales = "free_y", labeller = labeller(chains_assumption = chain_assumption_labs)) +
        geom_vline(xintercept = as.Date("2020-03-17"), linetype = "dashed") +
        geom_vline(xintercept = as.Date("2020-04-27"), linetype = "dashed") +
        scale_x_date(date_breaks = "1 month", date_labels = "%b.") +
        ...
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5), legend.position = "bottom") +
        labs(x = element_blank(), y = "Persistence from date (days)")
ggsave(filename = "figures/persisence_over_time_full_period.png",
       width = single_col_width, height = single_col_width, units = "cm")
```

Write out the persistence of introductions at the start of lockdown, post-lockdown peak
```{r}
lockdown_start_persistence <- persistence_results %>%
        filter(date == as.Date("2020-03-17"))
lockdown_end_persistence <- persistence_results %>%
