...
validation_results <- samples_with_foreign_exposure_vs_prior %>% 
  ...
  mutate(is_classified = total_asr_weight == 1)

p <- ggplot(
  data = validation_results %>% filter(is_classified),
  aes(x = prior_asr_weight, y = correct_asr_weight)) + 
  geom_count() +
  scale_size_area(name = "Number samples") + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") + 
  facet_grid(. ~ chains_assumption, labeller = labeller(chains_assumption = chain_assumption_labs)) + 
  shared_theme + 
  labs(x = "Fraction of travel context sequences matching exposure location",
       y = "Parsimony score of exposure location")
  # geom_text(aes(label = iso_country_exposure))
...
Switch to difference from prior

```{r}
ggplot(
  data = validation_results %>% filter(is_classified),
  aes(x = correct_asr_weight - prior_asr_weight)) + 
  geom_histogram() +
  facet_grid(. ~ chains_assumption, labeller = labeller(chains_assumption = chain_assumption_labs)) + 
  shared_theme + 
  labs(x = "Improvement over context set")
  # geom_text(aes(label = iso_country_exposure))
```
...
Is Serbia really 66% of some month's context set? In June, yes (there are only 9 seqs total, 6 are SRB).
TODO: What about prior weight in the right tree?
```{r}
tmp <- samples_with_foreign_exposure_vs_prior %>% 
  filter(chains_assumption == "max") %>%
  group_by(prior_month) %>% 
  mutate(total_inds = sum(n_inds, na.rm = T)) %>%
  group_by(prior_month, origin) %>%
  summarize(frac_inds = sum(n_inds, na.rm = T) / total_inds[1])
View(tmp)
...
# Number exposed samples
validation_results_3 %>% 
  group_by(chains_assumption) %>%
  summarise(
    n_exposed_samples = sum(n_foreign_exposures))
# Number chains with exposed samples
validation_results_3 %>%
  group_by(chains_assumption) %>%
  summarise(
    number_chains_with_exposed_sample = sum(n_foreign_exposures > 0))
# Number of chains with only one exposure location
validation_results_3 %>%
  group_by(chains_assumption) %>%
  summarise(
    number_chains_with_one_exp_location = sum(n_diff_foreign_exposures == 1))
...
validation_results_3 %>%
  ...
  summarize(
    n_chains = n(), 
    frac_chains = n() / n_chains_total[1])
```
