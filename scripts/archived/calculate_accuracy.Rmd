---
title: "Generate accuracy statistics for normal run"
author: "Sarah Nadeau"
date: "21/04/2021"
...
Normal run: without preference for samples with known BAG- or GISAID-recorded exposure location.
...
# TODO: doesn't plot exposure location right
```{r}
...
for (i in 1:nrow(trees_to_plot)) {
  ...
  lineage <- gsub(x = tree, pattern = "_m_3_p_1", replacement = "")
  ...
  tree_file <- paste(
    workdir, "/tmp/lsd/", lineage, ".timetree.nex", sep = "")
  tree_data_with_asr_file <- paste(
    workdir, "/tmp/asr/", tree, "_s_", chains_assumption_TF, "_tree_data_with_asr.txt", sep = "")
  chains_file <- paste(
    workdir, "/tmp/chains/", tree, "_s_", chains_assumption_TF, "_chains.txt", sep = "")
...
ggsave(
  plot = p,
  file = paste(workdir, "output", "accuracy_for_validation_samples.png", sep = "/"),
  width = 5,
  height = 3, 
  units = "in")
```
...
ggsave(
  plot = p,
  file = paste(workdir, "output", "posterior_to_prior_for_validation_samples_max.png", sep = "/"),
  width = 4, 
  height = 3, units = "in"
)
```
