...
write.csv(x = line_list_re_data, file = "data/CHE_re_data_used.csv")
```
...
start_plot_date <- as.Date("2020-01-01")  # plot to show prior
...
aligned_plots <- align_plots(samp_prop_plot, re_plot)
...
ggsave("figures/CHE_contact_tracing_factor.pdf", width = double_col_width, height = single_col_width * 0.75, units = "cm")
```
...
re_peak_noCT <- rbind(re_after_peak, re_peak) %>% filter(!contact_tracing)
write.table(x = re_peak_noCT, file = "manuscript/re_estimates.txt", row.names = F)
...
damping_factor_medians <- CT_data %>%
  ...
  summarize(
    median_damping_factor = median(value),
    lowHPD_damping_factor = quantile(value, 0.025),
    highHPD_damping_factor = quantile(value, 0.0975),
    median_transmission_rate_decrease = median(transmission_rate_decrease),
    lowHPD_transmission_rate_decrease = quantile(transmission_rate_decrease, 0.025),
    highHPD_transmission_rate_decrease = quantile(transmission_rate_decrease, 0.975),
    .groups = "drop"
  )
write.table(x = damping_factor_medians, file = "manuscript/damping_factor_variables.txt", row.names = F)
