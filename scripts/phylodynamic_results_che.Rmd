...
ggplot() +  
  ...
  scale_fill_manual(values = c("#009E73", "#E69F00"), labels = chain_assumption_labs, name = "Data set") + 
  shared_theme + 
...
## Selected analyses damping factor figure for main text
...
ggsave("figures/contact_tracing_factor_no_sampUB.pdf", width = single_col_width, height = single_col_width * 0.75, units = "cm")
```
...
summer_max_dampling_percent_median_CHE_no_sampUB <- round(unlist(dampling_factor_medians %>%
  filter(sampUB == "Without sampling bound", clusters == "max", season == "Summer") %>%
  select(median_dampling_factor)) * 100, digits = 0)

writeLines(text = paste0("\\newcommand\\summer_max_dampling_percent_median_CHE_no_sampUB{", 
                         summer_max_dampling_percent_median_CHE_no_sampUB, "}"), con = con)

summer_min_dampling_percent_median_CHE_no_sampUB <- round(unlist(dampling_factor_medians %>%
  filter(sampUB == "Without sampling bound", clusters == "min", season == "Summer") %>%
  select(median_dampling_factor)) * 100, digits = 0)

writeLines(text = paste0("\\newcommand\\summer_min_dampling_percent_median_CHE_no_sampUB{", 
                         summer_min_dampling_percent_median_CHE_no_sampUB, "}"), con = con)

close(con)

system("cp figures/contact_tracing_factor_no_sampUB.pdf ../../../../figures/")
system("cp figures/contact_tracing_factor.pdf ../../../../figures/")
```
