...
color_labels <- c("conf_case_counts" = "Confirmed\ncases", 
                  "TRUE" = "Model with\ndamping factor", 
                  "FALSE" = "Model without\ndamping factor")

re_plot <- ggplot(skyline_data %>% filter(variable=="Re") %>% left_join(Re_dates),
...
ggsave("figures/CHE_Re.pdf", width=double_col_width, height=single_col_width * 0.5, units="cm")

# Get the first ribbon to show by repeating the first interval value at the minimum date
samp_prop_data_1 <- skyline_data %>% filter(variable=="sampProp") %>% left_join(sampProp_dates)
max_interval <- max(samp_prop_data_1$interval)
samp_prop_data_2 <- samp_prop_data_1 %>% filter(interval == max_interval) %>% mutate(date = as.Date("2020-01-01"))
samp_prop_data <- rbind(samp_prop_data_1, samp_prop_data_2)

samp_prop_plot <- ggplot(samp_prop_data,
       aes(date, median, col=contact_tracing, fill=contact_tracing)) +
    tidybayes::geom_lineribbon(aes(ymin=low, ymax=high), alpha=0.5, step="vh") +
    ...
    shared_theme + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
ggsave("figures/CHE_sampProp.pdf", width=double_col_width, height=single_col_width * 0.5, units="cm")
```
