...
first_week_data <- samp_prop_data_1 %>% filter(interval == max_interval) %>% mutate(date = start_plot_date)
...
Dates are midpoint of interval
```{r}
...
write.csv(
  x = re_data_to_use,
  file = paste0(datadir, "/../re_by_week.csv"),
  row.names = F
)
```