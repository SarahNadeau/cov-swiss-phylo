...
knitr::opts_knit$set(root.dir = "/Users/nadeaus/Repos/cov-swiss-phylogenetics/results_all/bdsky/log_files_2_deme/SwissTransmissionChains")
knitr::opts_chunk$set(root.dir = "/Users/nadeaus/Repos/cov-swiss-phylogenetics/results_all/bdsky/log_files_2_deme/SwissTransmissionChains/")
```
...
for (clusters in c("min", "max")) {
  for (contact_tracing in c(TRUE, FALSE)) {
    for (sampUB in c("With sampling bound", "Without sampling bound")) {
      pattern <- paste0("Re_skyline_2deme.", clusters, "_chains",
...
## SS type frequencies, inflation factor
...
for (clusters in c("min", "max")) {
  for (contact_tracing in c(TRUE, FALSE)) {
    for (sampUB in c("With sampling bound", "Without sampling bound")) {
      ...
      SS_data <- bind_rows(SS_data,
                           data %>%
                             slice_tail(prop=0.9) %>%
                             select(Sample, "SSInflation", "typeFrequencies.1", "typeFrequencies.2") %>%
                             mutate(clusters=clusters,
                                    contact_tracing=contact_tracing,
                                    sampUB=sampUB))
    }
...
```
Plot SS inflation factor, type frequencies
```{r}

ggplot() +
    geom_histogram(
            data = SS_data %>% filter(contact_tracing == "TRUE", clusters == "max"),
            aes(x = SSInflation, fill = clusters), alpha = 0.5, bins = 30) +
    geom_histogram(
            data = SS_data %>% filter(contact_tracing == "TRUE", clusters == "min"),
            aes(x = SSInflation, fill = clusters), alpha = 0.5, bins = 30) +
    facet_grid(. ~ sampUB) +
    scale_fill_manual(values = chains_assumption_colors, labels = chain_assumption_labs, name = chains_assumption_legend_name) +
    shared_theme +
    labs(x = ss_factor_x_lab, y = ct_factor_y_lab)

ggsave("figures/super_spreading_factor.pdf", width = double_col_width, height = single_col_width * 0.75, units = "cm")

ggplot() +
   geom_histogram(
            data = SS_data %>% filter(contact_tracing == "TRUE", clusters == "max"),
            aes(x = typeFrequencies.1, fill = clusters), alpha = 0.5, bins = 30) +
    geom_histogram(
            data = SS_data %>% filter(contact_tracing == "TRUE", clusters == "min"),
            aes(x = typeFrequencies.1, fill = clusters), alpha = 0.5, bins = 30) +
    facet_grid(. ~ sampUB) +
    scale_fill_manual(values = chains_assumption_colors, labels = chain_assumption_labs, name = chains_assumption_legend_name) +
    shared_theme +
    labs(x = "Fraction of population with higher transmission rate", y = ct_factor_y_lab)

ggsave("figures/super_spreading_type_frequency.pdf", width = double_col_width, height = single_col_width * 0.75, units = "cm")
```
...
writeLines(text = paste0("\\newcommand\\summermaxdampingpercentmedianCHESSnosampUB{",
...
writeLines(text = paste0("\\newcommand\\summermindampingpercentmedianCHESSnosampUB{",
                         summer_min_damping_percent_median_CHE_no_sampUB, "}"), con = con)

close(con)

system("cp figures/contact_tracing_factor.pdf ../../../../figures/CHE_2Deme_contact_tracing_factor.pdf")
system("cp figures/sampProp.pdf ../../../../figures/CHE_2Deme_sampProp.pdf")
system("cp figures/Re.pdf ../../../../figures/CHE_2Deme_Re.pdf")
system("cp figures/super_spreading_factor.pdf ../../../../figures/CHE_2Deme_super_spreading_factor.pdf")
system("cp figures/super_spreading_type_frequency.pdf ../../../../figures/CHE_2Deme_super_spreading_type_frequency.pdf")
```
