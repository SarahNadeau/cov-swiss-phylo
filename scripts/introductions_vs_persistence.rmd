...
Implementation adapted from https://github.com/BEAST2-Dev/bdsky/blob/master/src/beast/evolution/speciation/BirthDeathSkylineModel.java
```{r}
# Load median Re estimates from BDSKY analysis
re_by_week <- read.csv(file = "results_all/bdsky/log_files/SwissTransmissionChains/re_by_week.csv")  %>%
        filter(clusters == "min") %>%
        arrange(date)

# Get time (in years) from start of process to start of each interval
breakpoint_days <- c(
        as.Date("2020-01-01"),  # start of process
        sort(as.Date(unique(re_by_week$date)[!is.na(unique(re_by_week$date))])) + 1,  # reported dates are end of each interval
        as.Date("2020-11-30")  # end of sampling period
)
times <- as.numeric(difftime(breakpoint_days, as.Date("2020-01-01"), units = "days") / 365)
m <- length(times) - 1
...
for (i in rev(0:(m - 1))) {
        print(paste("Calculating p0 for interval", i + 1))
        p0[i + 1] <- calc_p0(lambda[i + 1], mu, psi, Ai[i + 1], Bi[i + 1], times[i + 2], times[i + 1])  # note on time indices: interval 49 spands breakpoint_days[49] - breakpoint_days[50] 
        if (i > 0) {
                print(paste("Calculating Bi for interval", i))
                Bi[i] <- calc_Bi(lambda[i], mu, psi, rho, Ai[i], p0[i + 1])
        }
}
print(p0)

# Probability of being sampled is 1 - p0
print(1 - p0)

# Plot probability of an introduction being sampled over time
p0_results <- data.frame(
        interval_start = breakpoint_days[1:m],
        interval_end = re_by_week$date,
        prob_being_sampled = 1 - p0,
        re_median = re_by_week$median
)
ggplot(data = p0_results, aes(x = interval_start)) +
