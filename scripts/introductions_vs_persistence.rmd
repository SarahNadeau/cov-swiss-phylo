...
introduction_summary_v2 <- introduction_summary %>%
  ...
  mutate(
    which_most = chains_assumption[[1]],
...
for (idx in 1:2) {
  ...
  if (idx == 1) {
    ...
    p0_results <- rbind(p0_results, p0_results_tmp %>% 
      left_join(
        re_by_week_max %>% 
          mutate(week = as.Date(date), re_median = median) %>% 
          select(week, re_median)
...
end_plot_date <- as.Date("2020-07-01")
...
get_predictions <- function(chain_summary_stats, day_to_week, p0_results, cases_delay, cases_coeff, chains) {
  predictions <- chain_summary_stats %>%
    ...
    group_by(week) %>%
    summarise(introductions_by_week_raw = n(), .groups = "drop") %>%
    full_join(p0_results %>% mutate(week = as.Date(week)) %>% filter(chains_assumption == chains)) %>%
...
data_max <- get_predictions(
  chain_summary_stats, day_to_week, p0_results, cases_delay_max, cases_coeff_max, "max"
) %>% rename(introductions_by_week_max = introductions_by_week, predictions_max = predictions)

data_min <- get_predictions(
  chain_summary_stats, day_to_week, p0_results, cases_delay_min, cases_coeff_min, "min"
) %>% rename(introductions_by_week_min = introductions_by_week, predictions_min = predictions)

data_max_fit_after <- get_predictions(
  chain_summary_stats, day_to_week, p0_results, cases_delay_max_fit_after, cases_coeff_max_fit_after, "max"
) %>% rename(introductions_by_week_max = introductions_by_week, predictions_max = predictions)

data_min_fit_after <- get_predictions(
  chain_summary_stats, day_to_week, p0_results, cases_delay_min_fit_after, cases_coeff_min_fit_after, "min"
) %>% rename(introductions_by_week_min = introductions_by_week, predictions_min = predictions)

introductions_vs_indicence_data <- merge(data_max, data_min) %>% full_join(case_data_base)
# introductions_vs_indicence_data <- merge(data_max_fit_after, data_min_fit_after) %>% full_join(case_data_base)

introductions_vs_indicence_model <- ggplot(
  data = introductions_vs_indicence_data %>% filter(date <= end_plot_date),
  aes(x = date)
) +
  geom_errorbar(aes(
    ymin = introductions_by_week_min,
    ymax = introductions_by_week_max,
  ...
  scale_color_manual(
    values = c("Estimate range:\nfew to many introductions" = "black"),
    name = element_blank()
  ) +
  shared_theme +
  theme(
...
con <- file("manuscript/intoductions_by_incidence_model_fit_after.tex", open = "w")
...
right_col <- plot_grid(introductions_vs_indicence_model, persisence_over_time,
  nrow = 2, align = "v", axis = "tb", labels = c("C", "D")
)
left_col <- plot_grid(introductions_through_time, persistence_through_time,
  nrow = 2, align = "v", axis = "tb", labels = c("A", "B")
)
