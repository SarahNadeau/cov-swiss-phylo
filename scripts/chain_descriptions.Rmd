...
Plot introductions and persistance through time
```{r}
introductions_and_persistors_summary_stats <- chain_summary_stats %>%
  group_by(chains_assumption, first_sample_month) %>%
  summarise(n_introductions = n(),
            # frac_20_day_persistor = sum(time_to_last_sample >= 20) / n(),
            # frac_40_day_persistor = sum(time_to_last_sample >= 40) / n(),
            frac_60_day_persistor = sum(time_to_last_sample >= 60) / n(),
            # frac_80_day_persistor = sum(time_to_last_sample >= 80) / n(),
            .groups = "drop") %>%
   tidyr::pivot_longer(names_to = "statistic", cols = c("n_introductions", ends_with("_day_persistor"))) %>%
   tidyr::pivot_wider(names_from = "chains_assumption", values_from = "value") %>%
   mutate(grey_out = statistic == "frac_60_day_persistor" & first_sample_month %in% c("2020-10-01", "2020-11-01"),
          statistic = factor(statistic, levels = c("n_introductions", "frac_60_day_persistor")))

statistic_labels <- c("New introductions (number newly sampled per month)", "Persistent introductions (fraction still sampled after 60 days)")
names(statistic_labels) <- c("n_introductions", "frac_60_day_persistor")

write.csv(x = introductions_and_persistors_summary_stats, 
          file = paste(outdir, "introductions_and_persistors_summary_stats.csv", sep = "/"))

p <- ggplot(data = introductions_and_persistors_summary_stats,
            aes(x = as.Date(first_sample_month),
                alpha = grey_out)) +
  ...
  facet_wrap(
    ...
    scales = "free_y",
    ncol = 1,
    labeller = labeller(statistic = statistic_labels)) +
  labs(x = "Month first sampled", y = element_blank()) +
...
ggsave(
    plot = p,
    file = paste(outdir, "introductions_and_persistance.png", sep = "/"),
    width = single_col_width,
    height = single_col_width,
    units = "cm")
```
