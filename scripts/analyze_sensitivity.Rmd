...
load_results <- function(
  prefix = "results_all/2021-07-05_for_supplement_sim_",
  suffix = "_rep_",
  reps = c("1", "2", "3"),
  values = c("1", "2", "3")
) {
...
```{r}
prefix <- "2021-06-02_for_supplement_sim_"
...
subsample_fractions <- seq(from = 0.01, to = 0.05, by = 0.01)
n_reps <- 50
n_focal_sequences <- nrow(
  results$`2x_repMain analysis`$samples %>%
    filter(chains_assumption == "min"))
n_seqs_to_keep <- round(subsample_fractions * n_focal_sequences / max_sampling_frac)
full_samples <- results$`2x_repMain analysis`$samples

for (i in 1:length(subsample_fractions)) {
  for (j in 1:n_reps) {
    n_seqs <- n_seqs_to_keep[i]
    subsample <- full_samples %>% sample_n(size = n_seqs, replace = F)
    size_dist <- subsample %>% 
...
p <- ggplot(
  ...
  aes(x = paste0(subsample, "%;\nn=", n_focal_seqs))) + 
  geom_boxplot(aes(y = n_singletons, color = "Singletons")) +
  geom_boxplot(aes(y = n_chains, color = "Chains")) + 
  geom_boxplot(aes(y = n_chains + n_singletons, color = "Introductions")) + 
